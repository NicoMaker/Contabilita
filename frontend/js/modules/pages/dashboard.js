...
function _salvaFiltriDashboardSuStorage() {
  try {
    var keys = typeof window._activeFiltroKeys !== "undefined" 
      ? window._activeFiltroKeys 
      : new Set();
    var nessuno = typeof window._filtroManualeNessuno !== "undefined" 
...
function buildDashboardShell(stats) {
  document.getElementById("content").innerHTML = `
    ...
    <div style="margin-bottom:16px" onclick="event.stopPropagation()">
      ...
      <div id="dash-tip-filtro-container" style="display:none" onclick="event.stopPropagation()"></div>
    </div>
  ...
  setTimeout(function () {
    ...
    try {
      var saved = localStorage.getItem(_getStorageKeys().FILTRI);
      if (saved) {
        var filtriData = JSON.parse(saved);
    ...
    window.removeEventListener('filtriTipologieAggiornati', _handleFiltriAggiornati);
    window.addEventListener('filtriTipologieAggiornati', _handleFiltriAggiornati);
...
// ─── HANDLER EVENTI FILTRI ────────────────────────────────────
function _handleFiltriAggiornati(e) {
...
// ─── SANITIZZA BOTTONI PANNELLO ───────────────────────────────
function _sanitizePanelButtons(element) {
  if (!element) return;
  
  var buttons = element.querySelectorAll('[onclick]');
  for (var i = 0; i < buttons.length; i++) {
    ...
    if (onclickAttr) {
      ...
      btn.addEventListener('click', function(e) {
        ...
        e.preventDefault();
        try {
          eval(this.getAttribute('data-original-onclick') || onclickAttr);
        } catch(err) {
          console.warn('Errore esecuzione onclick:', err);
        }
      });
      btn.setAttribute('data-original-onclick', onclickAttr);
    }
  }
...
function renderClientiSenzaAdempimenti(clienti) {
  ...
  if (!clienti || clienti.length === 0) {
    container.innerHTML = '<div style="text-align:center;padding:20px;color:var(--green);background:var(--green)08;border-radius:8px">' +
      "✅ Tutti i clienti hanno almeno un adempimento per l'anno " + state.anno + "!" + "</div>";
    return;
  }
  ...
  for (var i = 0; i < clienti.length; i++) {
    ...
    html += '<div style="display:flex;align-items:center;justify-content:space-between;background:var(--s2);border:1px solid var(--b1);border-radius:10px;padding:12px 16px;gap:12px">' +
...
function getSelectedAdempimentiApplica() {
  return Array.from(document.querySelectorAll(".applica-adp-checkbox:checked")).map(function (cb) {
    return parseInt(cb.value);
  });
}

function getSelectedClientiApplica() {
  return Array.from(document.querySelectorAll(".applica-cliente-checkbox:checked")).map(function (cb) {
    return parseInt(cb.value);
  });
}
