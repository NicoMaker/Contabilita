  ...
  const tbody = clienti.length
    ? clienti
        .map((c) => {
          ...
          if (c.col2_value)
            infoBadges += ` <span class="badge-info" title="Sottocategoria: ${col2Map[c.col2_value] || c.col2_value}">${col2Map[c.col2_value] || c.col2_value}</span>`;
          if (c.col3_value)
            infoBadges += ` <span class="badge-info" title="Regime: ${col3Map[c.col3_value] || c.col3_value}">${col3Map[c.col3_value] || c.col3_value}</span>`;
          if (c.periodicita)
            infoBadges += ` <span class="badge-per" title="Periodicità: ${c.periodicita === "mensile" ? "Mensile" : "Trimestrale"}">${c.periodicita === "mensile" ? "📅" : "📆"} ${c.periodicita === "mensile" ? "Mens." : "Trim."}</span>`;
...
function showClienteDettaglio(id) {
  ...
  const catHtml = categorie
    .map((cat) => {
      ...
      return found
        ? `<span class="cat-det-badge" style="color:${found.color};border-color:${found.color}33;background:${found.color}11" title="${found.nome}: ${getCatDescription(found.codice)}">${found.icona} ${found.codice}</span>`
        : "";
    })
    .join("");
...
function validaClassificazioneCliente() {
  ...
  if (col2Wrap && col2Wrap.style.display !== "none") {
    ...
    if (!col2Value) {
      showNotif(
        "La Sottocategoria è obbligatoria per questa tipologia",
        "error",
      );
