...
function renderPage(page) {
  ...
  if (page === "dashboard") {
    ...
    document.getElementById("topbar-actions").innerHTML = `
      <div class="search-wrap" style="width:300px">
        <span class="search-icon">🔍</span>
        <input class="input" id="global-search-adempimenti" placeholder="Cerca codice, nome, descrizione..." oninput="applyAdempimentiFiltri()">
      </div>
      <button class="btn btn-print btn-sm no-print" onclick="window.print()">🖨️ Stampa</button>
...
function renderDashboard(stats) {
  ...
  document.getElementById("content").innerHTML = `
    ...
    <div class="stats-grid">
      <div class="stat-card"><div class="stat-label">Clienti Totali</div><div class="stat-value v-blue">${stats.totClienti}</div></div>
      <div class="stat-card"><div class="stat-label">Adempimenti ${stats.anno}</div><div class="stat-value">${stats.totAdempimenti}</div></div>
      <div class="stat-card"><div class="stat-label">Completati</div><div class="stat-value v-green">${stats.completati}</div><div class="prog-bar"><div class="prog-fill green" style="width:${perc}%"></div></div><div class="stat-sub">${perc}% del totale</div></div>
...
function renderClientiTabella(clienti) {
  ...
  document.getElementById("content").innerHTML = `
    ...
    <div class="table-wrap"><div class="table-header no-print"><h3>Clienti (${clienti.length})</h3></div>
...
function renderScadenzarioPage() {
  ...
  document.getElementById("topbar-actions").innerHTML = `
    ...
    <div class="search-wrap" style="width:220px">
      <span class="search-icon">🔍</span>
      <input class="input" id="scad-search" placeholder="Cerca adempimento..." oninput="applyScadFiltri()">
    </div>
  `;
...
function renderScadenzarioTabella(righe) {
  ...
  document.getElementById("content").innerHTML = `
    ...
    <div class="filtri-bar no-print"><label>Filtra stato:</label><select class="select" style="width:160px" id="f-stato" onchange="applyScadFiltri()"><option value="tutti">Tutti</option><option value="da_fare">⭕ Da fare</option><option value="in_corso">🔄 In corso</option><option value="completato">✅ Completato</option><option value="n_a">➖ N/A</option></select>
    <button class="btn btn-sm btn-secondary" onclick="resetScadFiltri()">✕ Reset</button></div>
    <div class="table-wrap"><div class="table-header no-print"><h3>Scadenzario ${state.anno}</h3></div>
...
function renderGlobaleFiltri() {
  document.getElementById("content").innerHTML = `
    ...
    <div class="filtri-bar no-print">
      <label>Stato:</label><select class="select" style="width:160px" id="fg-stato" onchange="applyGlobaleFiltri()">
...
function renderAdempimentiTabella(adempimenti) {
  ...
  document.getElementById("content").innerHTML = `
    ...
    <table><thead><tr><th>Codice</th><th>Nome</th><th>Descrizione</th><th>Categoria</th><th>Cadenza</th><th class="no-print">Azioni</th></tr></thead>
