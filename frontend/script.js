...
// ─── ROW STORE globale ────────────────────────────────────────────────────
let _rowStore = {};
function storeRow(r) { _rowStore[r.id] = r; return r.id; }
function openAdpById(id) {
  const r = _rowStore[id];
  if (!r) { console.warn('Row not found for id:', id); return; }
  openAdpModal(
    r.id, r.stato, r.data_scadenza||'', r.data_completamento||'',
    r.importo||'', r.note||'',
    r.adempimento_nome + ' – ' + getPeriodoLabel(r),
    r.is_contabilita||0, r.has_rate||0,
    r.importo_saldo||'', r.importo_acconto1||'', r.importo_acconto2||'',
    r.importo_iva||'', r.importo_contabilita||'', r.rate_labels||''
  );
}
...
let state = {
  ...
  scadenzario: [], scadGlobale: [], dashStats: null,
  dashSearch: '', dashFiltroCategoria: 'tutti',
  globalePreFiltroAdp: '',
  _dashRendered: false,
};
...
socket.on('res:tipologie', ({success,data}) => { if (success) { state.tipologie=data; populateTipologiaSelect(); } });
...
socket.on('res:create:adempimento', ({success,error}) => { if(success){closeModal('modal-adp-def');state._pending='adempimenti';socket.emit('get:adempimenti');}else showNotif(error,'error'); });
socket.on('res:update:adempimento', ({success,error}) => { if(success){closeModal('modal-adp-def');state._pending='adempimenti';socket.emit('get:adempimenti');}else showNotif(error,'error'); });
...
function changeAnno(d) {
  ...
  if(state.page==='dashboard') { state._dashRendered=false; socket.emit('get:stats',{anno:state.anno}); }
}

// ============================================================
// DASHBOARD
// FIX: la struttura HTML viene creata UNA SOLA VOLTA (buildDashboardShell).
// Ad ogni keystroke/filtro si aggiorna SOLO il tbody e le stat-card
// con updateDashboardContent — il campo <input> non viene mai rimosso/ricreato,
// quindi non perde il focus.
// ============================================================
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="table-wrap">
      <div class="table-header no-print" style="flex-wrap:wrap;gap:10px">
        <h3 id="dash-adp-count-title" style="white-space:nowrap">Adempimenti ${stats.anno}</h3>
        <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;flex:1">
          ...
          <div class="search-wrap" style="width:210px;margin-left:auto">
            ...
            <input class="input" id="dash-adp-search" placeholder="Cerca nome, codice..."
              value="${escAttr(state.dashSearch)}"
              oninput="onDashAdpSearch(this.value)">
          </div>
        </div>
      </div>
      <table>
...
function updateDashboardContent(stats) {
  ...
  const catColor = {}; CATEGORIE.forEach(c => catColor[c.codice]=c.color);
  ...
  const filtInCorso    = adpVis.reduce((s,a)=>s+Math.max(0,a.totale-a.completati-a.da_fare),0);
  const filtPerc       = filtTotale>0 ? Math.round((filtCompletati/filtTotale)*100) : 0;
  const isFiltered     = sc!=='tutti' || sq!=='';

  // Aggiorna stat-card senza toccare il DOM dell'input
  ...
  setTxt('ds-lbl-tot', `Adempimenti ${stats.anno}${isFiltered?' <span style="font-size:9px;color:var(--yellow)">(filtro)</span>':''}`);
  setEl('ds-tot', filtTotale);
  setEl('ds-comp', filtCompletati);
  setEl('ds-dafare', filtDaFare);
  setEl('ds-incorso', filtInCorso);
  setEl('ds-na', stats.na||0);
  setEl('ds-perc', filtPerc+'%');
  setW('ds-prog', filtPerc+'%');

  // Aggiorna titolo
  const title = document.getElementById('dash-adp-count-title');
  if(title) title.innerHTML = `Adempimenti ${stats.anno} <span style="font-size:11px;font-weight:400;color:var(--text3);margin-left:6px">${adpVis.length}/${allAdp.length} — clicca riga per Vista Globale</span>`;

  // Aggiorna cat-tabs (i pulsanti, non il campo input)
  const tabsEl = document.getElementById('dash-cat-tabs');
  if(tabsEl) {
    tabsEl.innerHTML = [{codice:'tutti',nome:'Tutti',color:'var(--text2)'},...CATEGORIE].map(c => {
      ...
      return `<button class="cat-tab${active?' cat-tab-active':''}"
        ...
        onclick="setDashCat('${c.codice}')">${c.nome||c.codice}</button>`;
    }).join('');
  }

  // Aggiorna SOLO il tbody — l'input #dash-adp-search non viene mai toccato
  const tbody = document.getElementById('dash-adp-tbody');
  if(!tbody) return;
  if(!adpVis.length) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:30px;color:var(--text3)">Nessun adempimento trovato</td></tr>`;
    return;
  }
  tbody.innerHTML = adpVis.map(a => {
    const p = a.totale>0 ? Math.round((a.completati/a.totale)*100) : 0;
    const inCorso = Math.max(0, a.totale - a.completati - a.da_fare);
    const dot = catColor[a.categoria] ? `<span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:${catColor[a.categoria]};margin-right:5px;vertical-align:middle"></span>` : '';
...
function onDashAdpSearch(val) {
  ...
  // Aggiorna subito senza rebuild — il campo input mantiene il focus
  if(state.dashStats) updateDashboardContent(state.dashStats);
}
...
function onClienteChange() { const id=parseInt(document.getElementById('sel-cliente').value); state.selectedCliente=state.clienti.find(c=>c.id===id)||null; if(state.selectedCliente) loadScadenzario(); }
...
function renderImportoCell(r) {
  ...
  if(r.has_rate){
    let labels=['Saldo','1°Acc.','2°Acc.']; try{if(r.rate_labels)labels=JSON.parse(r.rate_labels);}catch(e){}
  ...
  const tbody=Object.entries(grouped).map(([adpId,rows])=>{
    ...
    if(isMulti){
      const periodiHtml=rows.map(rx=>{
        const pl=r0.scadenza_tipo==='mensile'?MESI_SHORT[(rx.mese||1)-1]:`T${rx.trimestre}`;
      ...
      const totPeriodi=r0.scadenza_tipo==='mensile'?'12 mesi':'4 trimestri';
      return `<tr>${adpLabel}<td colspan="5"><div style="display:flex;align-items:flex-start;gap:12px;flex-wrap:wrap">
        <div style="flex:1"><div style="font-size:10px;color:var(--text3);margin-bottom:6px">${totPeriodi} — clicca un periodo per modificare</div>
        <div style="display:flex;flex-wrap:wrap;gap:3px">${periodiHtml}</div></div>
        <div style="text-align:right;min-width:80px"><div style="font-size:11px;color:var(--text2);margin-bottom:4px">${compR}/${totR}</div>
        <div class="prog-bar" style="width:80px"><div class="prog-fill green" style="width:${percR}%"></div></div>
        <div style="font-size:10px;color:var(--text3);margin-top:3px">${percR}%</div></div></div></td></tr>`;
    }
    return rows.map((rx,idx)=>{
      const periodo=getPeriodoLabel(rx);
      return `<tr class="clickable s-${rx.stato}" onclick="openAdpById(${rx.id})">${idx===0?adpLabel:'<td style="border-right:1px solid var(--border)"></td>'}
        <td>${periodo}</td><td><span class="badge b-${rx.stato}">${STATI[rx.stato]||rx.stato}</span></td>
        ...
        <td class="td-dim" style="font-size:11px;max-width:100px;overflow:hidden;text-overflow:ellipsis">${rx.note||''}</td></tr>`;
    }).join('');
  }).join('')||`<tr><td colspan="7"><div class="empty"><div class="empty-icon">&#128203;</div><p>Nessun adempimento. Clicca Genera.</p></div></td></tr>`;
...
function generaScad(id){if(confirm(`Generare scadenzario ${state.anno}?`))socket.emit('genera:scadenzario',{id_cliente:id,anno:state.anno});}
...
function openCopiaTutti(){
  document.getElementById('copia-cliente-id').value='';document.getElementById('copia-modalita').value='tutti';
  document.getElementById('copia-info').innerHTML='Copia per tutti i clienti.';
  document.getElementById('copia-da').value=state.anno-1;document.getElementById('copia-a').value=state.anno;openModal('modal-copia');
}
function eseguiCopia(){
  const modalita=document.getElementById('copia-modalita').value;
  const anno_da=parseInt(document.getElementById('copia-da').value);const anno_a=parseInt(document.getElementById('copia-a').value);
...
// ============================================================
// VISTA GLOBALE
// FIX CRITICO: storeRow per TUTTE le righe PRIMA di generare HTML
// Così quando l'utente clicca "Annuale" il dato è già nel _rowStore
// ============================================================
function renderGlobalePage() {
  ...
  document.getElementById('topbar-actions').innerHTML=`
    <div class="search-wrap" style="width:260px"><span class="search-icon">&#128269;</span>
      <input class="input" id="global-search-globale" placeholder="Cerca cliente, adempimento..."
        value="${escAttr(preSearch)}" oninput="onGlobaleSearchInput(this.value)">
    </div>
...
function clearGlobalePrefiltro(){state.globalePreFiltroAdp='';renderGlobalePage();}
function onGlobaleSearchInput(val){state.globalePreFiltroAdp=val;applyGlobaleSearchDB();}
function loadGlobale(searchOverride){
  const search=searchOverride!==undefined?searchOverride:(document.getElementById('global-search-globale')?.value||'');
...
function renderGlobaleTabella(righe){
  // FIX: prima store tutte le righe, poi genera HTML
  righe.forEach(r=>storeRow(r));
  ...
  const perCliente={};
  righe.forEach(r=>{
    if(!perCliente[r.id_cliente])perCliente[r.id_cliente]={id:r.id_cliente,nome:r.cliente_nome,tipo:r.tipologia_codice,righe:[]};
  ...
  const html=Object.values(perCliente).map(cl=>{
    ...
    const adpRows=Object.values(groupedAdp).map(adp=>{
      ...
      let periodoCell;
      if(isMulti){
        const sorted=adp.righe.slice().sort((a,b)=>adp.scad==='mensile'?(a.mese||0)-(b.mese||0):(a.trimestre||0)-(b.trimestre||0));
        const pills=sorted.map(rx=>{
          const pl=adp.scad==='mensile'?MESI_SHORT[(rx.mese||1)-1]:`T${rx.trimestre}`;
        ...
        // FIX: riga annuale/semestrale — già nel rowStore, onclick diretto sul badge
        const rx=adp.righe[0];
        periodoCell=`<span class="badge b-${rx.stato}" style="cursor:pointer;font-size:9px;padding:3px 10px;display:inline-block" onclick="openAdpById(${rx.id})">${getPeriodoLabel(rx)}</span>`;
      }
      return `<tr>
        ...
        <td><div style="display:flex;align-items:center;gap:6px">
          <div class="mini-bar" style="width:60px"><div class="mini-fill" style="width:${p2}%"></div></div>
          <span style="font-size:10px;font-family:var(--mono);color:var(--text3)">${p2}%</span>
        </div></td>
      </tr>`;
    }).join('');
...
// ADEMPIMENTI
// ============================================================
const applyAdpFiltriDB=debounce(()=>{const q=document.getElementById('global-search-adempimenti')?.value||'';socket.emit('get:adempimenti',{search:q});},300);
...
function renderAdempimentiTabella(adempimenti){
  const rows=adempimenti.map(a=>{
    ...
    const scadLabel={annuale:'1x/anno',semestrale:'2x/anno',trimestrale:'4x/anno',mensile:'12x/anno'};
    return `<tr>
...
function renderTipologiePage(){
  ...
  const cards=state.tipologie.map(t=>{
    ...
    const subs=(t.sottotipologie||[]).map(s=>{
      if(s.is_separator)return `<div style="font-size:10px;color:var(--text3);margin:8px 0 4px;font-weight:700;text-transform:uppercase">${s.nome}</div>`;
...
// MODAL CLIENTE
// ============================================================
function populateTipologiaSelect(){const sel=document.getElementById('c-tipologia');if(!sel)return;sel.innerHTML=state.tipologie.map(t=>`<option value="${t.id}">${t.codice} - ${t.nome}</option>`).join('');onTipologiaChange();}
function onTipologiaChange(){const tipId=parseInt(document.getElementById('c-tipologia')?.value);const tip=state.tipologie.find(t=>t.id===tipId);const sel=document.getElementById('c-sottotipologia');if(!sel||!tip)return;sel.innerHTML='<option value="">-- Nessuna --</option>'+(tip.sottotipologie||[]).map(s=>{if(s.is_separator)return `<option disabled>-- ${s.nome} --</option>`;return `<option value="${s.id}">${s.nome}</option>`;}).join('');}
function renderCategorieSelect(attuali=[]){const container=document.getElementById('categorie-attive');if(!container)return;container.innerHTML=CATEGORIE.map(cat=>`<label style="display:inline-flex;align-items:center;gap:8px;margin:4px 8px 4px 0;padding:8px 12px;background:var(--surface2);border-radius:var(--r-sm);cursor:pointer;border:1px solid ${attuali.includes(cat.codice)?'var(--accent)':'var(--border)'};transition:all 0.12s"><input type="checkbox" value="${cat.codice}" ${attuali.includes(cat.codice)?'checked':''} style="accent-color:var(--accent)" onchange="this.parentElement.style.borderColor=this.checked?'var(--accent)':'var(--border)'"><span>${cat.icona} ${cat.nome}</span></label>`).join('');}
...
function openNuovoCliente(){document.getElementById('modal-cliente-title').textContent='Nuovo Cliente';document.getElementById('cliente-id').value='';['c-nome','c-cf','c-piva','c-email','c-tel','c-indirizzo','c-note'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});renderCategorieSelect(['IVA','DICHIARAZIONI','PREVIDENZA','LAVORO','TRIBUTI','BILANCIO']);populateTipologiaSelect();openModal('modal-cliente');}
function editCliente(id){
  socket.once('res:cliente',({success,data})=>{
    if(!success||!data)return;
    document.getElementById('modal-cliente-title').textContent='Modifica Cliente';
    document.getElementById('cliente-id').value=data.id;document.getElementById('c-nome').value=data.nome||'';document.getElementById('c-cf').value=data.codice_fiscale||'';document.getElementById('c-piva').value=data.partita_iva||'';document.getElementById('c-email').value=data.email||'';document.getElementById('c-tel').value=data.telefono||'';document.getElementById('c-indirizzo').value=data.indirizzo||'';document.getElementById('c-note').value=data.note||'';
    const cat=JSON.parse(data.categorie_attive||'["IVA","DICHIARAZIONI","PREVIDENZA","LAVORO","TRIBUTI","BILANCIO"]');renderCategorieSelect(cat);populateTipologiaSelect();document.getElementById('c-tipologia').value=data.id_tipologia;onTipologiaChange();setTimeout(()=>{document.getElementById('c-sottotipologia').value=data.id_sottotipologia||'';},60);openModal('modal-cliente');
  });
  socket.emit('get:cliente',{id});
}
function saveCliente(){
  const id=document.getElementById('cliente-id').value;
  const data={nome:document.getElementById('c-nome').value.trim(),id_tipologia:parseInt(document.getElementById('c-tipologia').value),id_sottotipologia:document.getElementById('c-sottotipologia').value||null,codice_fiscale:document.getElementById('c-cf').value.trim().toUpperCase()||null,partita_iva:document.getElementById('c-piva').value.trim()||null,email:document.getElementById('c-email').value.trim()||null,telefono:document.getElementById('c-tel').value.trim()||null,indirizzo:document.getElementById('c-indirizzo').value.trim()||null,note:document.getElementById('c-note').value.trim()||null,categorie_attive:getSelectedCategorie()};
  ...
  if(id){data.id=parseInt(id);socket.emit('update:cliente',data);}else socket.emit('create:cliente',data);
}
function deleteCliente(id){if(confirm('Eliminare questo cliente?'))socket.emit('delete:cliente',{id});}
function goScadenzario(id){
  const c=state.clienti.find(x=>x.id===id);
  if(c){state.selectedCliente=c;document.querySelectorAll('.nav-item').forEach(x=>x.classList.remove('active'));document.querySelector('[data-page="scadenzario"]').classList.add('active');renderPage('scadenzario');}
  else{state._gotoClienteId=id;state._pending='scadenzario';socket.emit('get:clienti');}
}
...
function saveAdpStato(){
  ...
  const payload={id:parseInt(document.getElementById('adp-id').value),stato:document.getElementById('adp-stato').value,data_scadenza:document.getElementById('adp-scadenza').value||null,data_completamento:document.getElementById('adp-data').value||null,note:document.getElementById('adp-note').value||null,importo:null,importo_saldo:null,importo_acconto1:null,importo_acconto2:null,importo_iva:null,importo_contabilita:null};
...
// ============================================================
function onAdpFlagsChange(){const hasCont=document.getElementById('adp-def-contabilita').checked;const hasRate=document.getElementById('adp-def-rate').checked;if(hasCont)document.getElementById('adp-def-rate').checked=false;if(hasRate)document.getElementById('adp-def-contabilita').checked=false;document.getElementById('sect-rate-labels').style.display=document.getElementById('adp-def-rate').checked?'':'none';}
function openNuovoAdpDef(){document.getElementById('modal-adp-def-title').textContent='Nuovo Adempimento';['adp-def-id','adp-def-codice','adp-def-nome','adp-def-desc'].forEach(id=>document.getElementById(id).value='');document.getElementById('adp-def-scadenza').value='annuale';document.getElementById('adp-def-categoria').value='TUTTI';document.getElementById('adp-def-contabilita').checked=false;document.getElementById('adp-def-rate').checked=false;document.getElementById('adp-rate-l1').value='Saldo';document.getElementById('adp-rate-l2').value='1° Acconto';document.getElementById('adp-rate-l3').value='2° Acconto';document.getElementById('sect-rate-labels').style.display='none';openModal('modal-adp-def');}
function editAdpDef(id){const a=state.adempimenti.find(x=>x.id===id);if(!a)return;document.getElementById('modal-adp-def-title').textContent='Modifica Adempimento';document.getElementById('adp-def-id').value=a.id;document.getElementById('adp-def-codice').value=a.codice;document.getElementById('adp-def-nome').value=a.nome;document.getElementById('adp-def-desc').value=a.descrizione||'';document.getElementById('adp-def-scadenza').value=a.scadenza_tipo||'annuale';document.getElementById('adp-def-categoria').value=a.categoria||'TUTTI';document.getElementById('adp-def-contabilita').checked=!!a.is_contabilita;document.getElementById('adp-def-rate').checked=!!a.has_rate;if(a.rate_labels){try{const l=JSON.parse(a.rate_labels);document.getElementById('adp-rate-l1').value=l[0]||'Saldo';document.getElementById('adp-rate-l2').value=l[1]||'1° Acconto';document.getElementById('adp-rate-l3').value=l[2]||'2° Acconto';}catch(e){}}document.getElementById('sect-rate-labels').style.display=a.has_rate?'':'none';openModal('modal-adp-def');}
function saveAdpDef(){const id=document.getElementById('adp-def-id').value;const isC=document.getElementById('adp-def-contabilita').checked;const hasR=document.getElementById('adp-def-rate').checked;const data={codice:document.getElementById('adp-def-codice').value.trim().toUpperCase(),nome:document.getElementById('adp-def-nome').value.trim(),descrizione:document.getElementById('adp-def-desc').value.trim()||null,scadenza_tipo:document.getElementById('adp-def-scadenza').value,categoria:document.getElementById('adp-def-categoria').value,is_contabilita:isC,has_rate:hasR,rate_labels:hasR?[document.getElementById('adp-rate-l1').value||'Saldo',document.getElementById('adp-rate-l2').value||'1° Acconto',document.getElementById('adp-rate-l3').value||'2° Acconto']:null};if(!data.codice||!data.nome){showNotif('Codice e Nome obbligatori','error');return;}if(id){data.id=parseInt(id);socket.emit('update:adempimento',data);}else socket.emit('create:adempimento',data);}
function deleteAdpDef(id){const a=state.adempimenti.find(x=>x.id===id);if(confirm(`Eliminare "${a?.nome||id}"?`))socket.emit('delete:adempimento',{id});}

function openCopia(id){document.getElementById('copia-cliente-id').value=id;document.getElementById('copia-modalita').value='cliente';document.getElementById('copia-info').innerHTML='Copia scadenzario per questo cliente.';document.getElementById('copia-da').value=state.anno-1;document.getElementById('copia-a').value=state.anno;openModal('modal-copia');}
function openAddAdp(id){document.getElementById('add-adp-cliente-id').value=id;document.getElementById('add-adp-anno').value=state.anno;const sel=document.getElementById('add-adp-select');sel.innerHTML=state.adempimenti.map(a=>`<option value="${a.id}">[${a.categoria}] ${a.codice} - ${a.nome}</option>`).join('');updatePeriodoOptions();openModal('modal-add-adp');}
function updatePeriodoOptions(){const selAdp=document.getElementById('add-adp-select'),selP=document.getElementById('add-adp-periodo');if(!selAdp||!selP)return;const adp=state.adempimenti.find(a=>a.id===parseInt(selAdp.value));if(!adp)return;let opts='<option value="">- Seleziona -</option>';if(adp.scadenza_tipo==='trimestrale')opts+=`<option value="trimestre_1">1° Trim.</option><option value="trimestre_2">2° Trim.</option><option value="trimestre_3">3° Trim.</option><option value="trimestre_4">4° Trim.</option>`;else if(adp.scadenza_tipo==='semestrale')opts+=`<option value="semestre_1">1° Semestre</option><option value="semestre_2">2° Semestre</option>`;else if(adp.scadenza_tipo==='mensile')MESI.forEach((m,i)=>{opts+=`<option value="mese_${i+1}">${m}</option>`;});else opts+=`<option value="annuale">Annuale</option>`;selP.innerHTML=opts;}
function eseguiAddAdp(){const pv=document.getElementById('add-adp-periodo').value;let trimestre=null,semestre=null,mese=null;if(pv.startsWith('trimestre_'))trimestre=parseInt(pv.split('_')[1]);else if(pv.startsWith('semestre_'))semestre=parseInt(pv.split('_')[1]);else if(pv.startsWith('mese_'))mese=parseInt(pv.split('_')[1]);socket.emit('add:adempimento_cliente',{id_cliente:parseInt(document.getElementById('add-adp-cliente-id').value),id_adempimento:parseInt(document.getElementById('add-adp-select').value),anno:parseInt(document.getElementById('add-adp-anno').value)||state.anno,trimestre,semestre,mese});}

function openModal(id){document.getElementById(id).classList.add('open');}
function closeModal(id){document.getElementById(id).classList.remove('open');}
document.querySelectorAll('.modal-overlay').forEach(el=>{el.addEventListener('click',e=>{if(e.target===el)el.classList.remove('open');});});

function showNotif(msg,type='info'){const icons={success:'ok',info:'i',error:'!'};const el=document.createElement('div');el.className=`notif ${type}`;el.innerHTML=`<span>${icons[type]||'.'}</span><span>${msg}</span>`;document.getElementById('notif-container').appendChild(el);setTimeout(()=>el.remove(),3800);}